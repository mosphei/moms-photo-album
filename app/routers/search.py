...
def fuzz_people(q: str, db: Session):
    query = (
        select(SearchPersonModel)
        .outerjoin(PersonModel, and_(PersonModel.id == SearchPersonModel.person_id))
        .where(SearchPersonModel.date_seen < PersonModel.date_updated)
    )
...
async def search_people(
    ...
    query = (
        ...
        .outerjoin(SearchPersonModel, PersonModel.id == SearchPersonModel.person_id)
        .where(
            and_(
    ...
    paginated_response = PaginatedResults[PersonSearchResult](
        items=person_list, total_count=total_count, offset=offset, limit=limit
    )
...
def fuzz_photos(q: str, filter_conditions, db: Session):
    ...
    query = (
        select(SearchPhotoModel)
        .outerjoin(PhotoModel, and_(PhotoModel.id == SearchPhotoModel.photo_id))
        .where(SearchPhotoModel.date_seen < PhotoModel.date_updated)
    )
    outdated = db.execute(query).scalars().all()
    for spm in outdated:
        db.delete(spm)
    query = (
        ...
        .outerjoin(
            SearchPhotoModel,
            and_(PhotoModel.id == SearchPhotoModel.photo_id, SearchPhotoModel.q == q),
        )
        .where(
            and_(
                or_(
    ...
    for photo in photos_to_process:
        ...
        if photo.description is not None and len(photo.description) > 0:
            srcresult.relevance = compare(
                q, photo.description, processor=utils.default_process
            )
        else:
...
async def search_images(
    ...
    limit: int = 100,
    sortBy: Literal[
        "date_taken", "date_uploaded", "date_updated", "relevance"
    ] = "relevance",
...
async def get_test(s1: str, s2: str):
    return {
        ...
        "token_set_ratio": fuzz.token_set_ratio(
            s1, s2, processor=utils.default_process
        ),
