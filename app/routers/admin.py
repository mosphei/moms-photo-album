...
from app.media_utils import extract_creation_time, get_content_type_by_extension, get_date_from_filename, get_image_date, get_metadata_ffmpeg_python, make_photo_from_file
...
def get_user_list(current_user: User = Depends(get_current_user), db: Session = Depends(get_db), offset: int = 0, limit: int = 10):
    assert_admin(current_user)
...
def get_user(username: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    assert_admin(current_user)
