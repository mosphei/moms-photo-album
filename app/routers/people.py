...
async def get_people_list(
    ...
    if q is not None and len(q) > 1:
        ...
        items_stmt = (
            ...
            .offset(offset)
            .limit(limit)
            .order_by(SearchPersonModel.relevance.desc(), sort)
        )
        # count
        subquery = select(SearchPersonModel.person_id).where(
            and_(
                SearchPersonModel.q == term,
                SearchPersonModel.relevance >= MIN_RELEVANCE,
            )
...
def get_person_by_id(
    ...
    db_person = (
        db.query(PersonCountModel).filter(PersonCountModel.id == person_id).first()
    )
